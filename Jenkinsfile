...
pipeline {
    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                script {
                    ...
                    // Deploy to Minikube using kubeconfig file
                    withKubeConfig([credentialsId: MINIKUBE_KUBECONFIG_CREDENTIALS, kubeconfigFile: kubeconfigContent]) {
                        sh 'kubectl apply -f ./path/to/kubernetes-manifests/ --validate=false'
                    }
                }
            }
        }
        ...
        stage('Test') {
            steps {
                // Optional: Run tests against deployed application
                sh "kubectl exec <pod-name> -- curl http://localhost:8080/healthcheck"
            }
        }
    }
    ...
    post {
        always {
            // Clean up Minikube resources after deployment
            sh "${MINIKUBE_BIN} delete"
        }
    }
}
